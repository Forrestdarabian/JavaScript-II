...
// Problem 1: The event coordinator wants a list of the company names represented, use .forEach() to create a list of company names.
let company_name = [];
runners.forEach (function (runner){
    company_name.push (runner.id + " " + runner.company_name);
});
console.log(company_name)
// Problem 2: The event date has been changed and the coordinator needs to inform everyone immediately via email. Use .filter() to list the emails.
let email = runners.map((runner) => runner.email);
console.log(email);
// Problem 3: The donations ended up being counterfeit so the community center needs it all refunded from the refundedprice using .reduce()
